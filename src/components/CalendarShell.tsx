...
interface CalendarShellProps {
  ...
  gridAnimClass?: string;
}

export default function CalendarShell({
  month, year, onPrev, onNext,
  children, notesSlot, gridAnimClass = "",
}: CalendarShellProps) {
  return (
    <div className="w-full max-w-5xl mx-auto">
      ...
      <div className="flex justify-center">
        <div className="w-[90%] h-6 flex items-center justify-around px-6 rounded-t-xl"
          style={{ background: "linear-gradient(180deg,#e2e8f0 0%,#cbd5e1 100%)", boxShadow: "0 2px 8px rgba(0,0,0,0.10)" }}>
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i}
              className="w-3.5 h-3.5 rounded-full border-2 border-[#94a3b8]"
      ...
      <div className="bg-white rounded-b-3xl overflow-hidden"
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="order-2 lg:order-1 lg:w-52 xl:w-60 flex-shrink-0
            border-t lg:border-t-0 lg:border-r border-[var(--calendar-border)] bg-[#fafbfc]">
            <div className="p-4 lg:p-5 h-full">{notesSlot}</div>
          </div>

          {/* Grid area */}
          <div className={`order-1 lg:order-2 flex-1 p-5 lg:p-7 overflow-hidden ${gridAnimClass}`}>
            {children}
          </div>
